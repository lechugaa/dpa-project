...
class AequitasTask(luigi.Task):
    ...
    training = luigi.BoolParameter(default=True)  # confuso: en una parte requiere que sea True y en otra False
    ...
    def run(self):
        ...
        # execute process
        fair = MrFairness(historic=self.historic, query_date=self.query_date, training=self.training)
        fair.save_info()
    ...
    def output(self):
        ...
        upload_path = get_upload_path(
            historic=self.historic, query_date=self.query_date, prefix=DataEngineer.prefix, training=self.training)
